# Politika privatnosti — NSK Ranked

Zadnje ažuriranje: 10. svibnja 2026.

## Što prikupljamo

NSK Ranked prikuplja sljedeće podatke:
- Email adresa (za prijavu)
- Ime i nadimak (koje sami unosite)
- Profilna slika (opcionalno)
- Vrijeme provedeno u knjižnici (check-in/check-out sesije)

## Kako koristimo podatke

Podatke koristimo isključivo za:
- Prikaz rang liste korisnika
- Praćenje vaših sesija u knjižnici
- Gamifikaciju učenja (streakovi, achievementi, leveli)

## Dijeljenje podataka

Vaši podaci se **ne dijele** s trećim stranama.
Rang lista je javna unutar aplikacije — ostali korisnici vide vaše ime, nadimak i sate provedene u knjižnici.

## Pohrana podataka

Podaci se pohranjuju na Google Firebase platformi (EU region).

## Brisanje podataka

Za brisanje računa i svih podataka kontaktirajte nas na:
josipduricvadinjof@gmail.com

## Kontakt

Josip Đurić Vadinjof
josipduricvadinjof@gmail.com
